...
public class Professor_manage_class_click extends AppCompatActivity {
    ...
    private TextView tvYearAndBlock, tvClassDetails;
    private LinearLayout studentListContainer;
    private Button addstudent, takeattendance, history;
            ...
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                ...
                if (snapshot.exists()) {
                    ...
                    for (DataSnapshot Classsnapshot : snapshot.getChildren()) {
                        ...
                        LinearLayout.LayoutParams avatarParams = new LinearLayout.LayoutParams(80, 80); // Smaller size
                        ...
                        editIcon.setOnClickListener(v -> {
                            ...
                            AlertDialog dialog = new AlertDialog.Builder(Professor_manage_class_click.this)
                                    ...
                                    .setMessage("Edit the name of the student.")
                                    .setView(editText)
                            ...
                            public void onDataChange(@NonNull DataSnapshot attendanceSnapshot) {
                                if (attendanceSnapshot.exists()) {
                                    ...
                                    switch (attendanceStatus) {
                                        case "Present":
                                            studentLayout.setBackgroundColor(Color.GREEN);
                                            studentView.setTextColor(Color.WHITE);
                                            break;
                                        case "Late":
                                            studentLayout.setBackgroundColor(Color.parseColor("#FFA500")); // Orange
                                            studentView.setTextColor(Color.WHITE);
                                            break;
                                        case "Absent":
                                            studentLayout.setBackgroundColor(Color.RED);
                                            studentView.setTextColor(Color.WHITE);
                                            break;
                                    }
                                }
    ...
    private void showAddStudentDialog() {
        // Create a dialog box for adding a student
        AlertDialog.Builder builder = new AlertDialog.Builder(this);
        builder.setTitle("Add Student");

        // Create an EditText for the email input
        final EditText input = new EditText(this);
        input.setHint("Enter the Name of Student");

        builder.setView(input);

        builder.setPositiveButton("Next", null);  // Set null for custom handling
                ...
                if (!rawname.isEmpty()) {
                    ...
                    if (formattedName != null){
                        checkIfNameExists(formattedName, (exists) -> {
                            if (!exists) {
                                try {
                                    addStudentToDatabase(formattedName, dialog, input);
                                } catch (Exception e){
                        ...
                        input.setError("Invalid format! Please use 'Surname, Firstname'.");
                        input.requestFocus();
                    }
                } else {
                    Toast.makeText(this, "Please enter a valid name.", Toast.LENGTH_SHORT).show();
                    input.setError("Please input valid Name");
                    input.requestFocus();
                }
            });
        });
    ...
    private void addStudentToDatabase(String fullname, AlertDialog dialog, EditText input) {
        ...
        try {
            ...
            if (endTimeDate != null) {
                ...
                if (delay > 0) {
                    ...
                    Toast.makeText(this, "QR code deletion scheduled in " + delay / 1000 + " seconds.", Toast.LENGTH_SHORT).show();
                } else {
                    Log.d("QRCodeDebug", "End time has already passed.");
                    Toast.makeText(this, "End time has already passed. Deleting QR code now.", Toast.LENGTH_SHORT).show();
                    deleteQRCode();
                }
            }
