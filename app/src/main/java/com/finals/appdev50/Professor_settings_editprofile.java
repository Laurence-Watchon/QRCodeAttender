...
public class Professor_settings_editprofile extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        try {
            ...
            editMobileNumber.setText("09"); // Initialize with "09"
            editMobileNumber.setSelection(editMobileNumber.getText().length()); // Set cursor position at the end
            editMobileNumber.addTextChangedListener(new TextWatcher() {
                ...
                public void onTextChanged(CharSequence s, int start, int before, int count) {
                    ...
                    if (!s.toString().startsWith("09")) {
                        editMobileNumber.setText("09");
                        editMobileNumber.setSelection(editMobileNumber.getText().length()); // Keep cursor at the end
                    }
                }
            ...
            editMobileNumber.setOnTouchListener((v, event) -> {
                if (event.getAction() == MotionEvent.ACTION_UP) {
                    if (editMobileNumber.getSelectionStart() < 2) {
